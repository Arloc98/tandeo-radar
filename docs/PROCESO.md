# Cómo se construyó Tandeo Radar

*Complemento del pitch. Documenta el método, las herramientas y —sobre todo— lo que salió mal.
Cada cifra de aquí salió de una corrida real o de un registro del repositorio.*

## 1. El planteamiento: dos agentes con papeles distintos
...
La regla que da sentido a todo: **el orquestador escribe la prueba antes de despachar la
tarea.** El constructor recibe un contrato ejecutable, no una descripción en prosa. "Cumple los
criterios" deja de depender de que alguien lea con cuidado.

Cada tarea se despacha a su propia rama, con un modelo elegido según el riesgo: barato para lo
mecánico, caro para lo que puede romper el producto en silencio.
...
- **Claude Code** como orquestador, con automatización de Chrome para verificar la interfaz en
  el navegador real en lugar de suponer que funciona.
...
Esta es la sección que importa. Ordenada por capa.
...
**Peor:** Lightning a veces deja `reasoning_content` vacío y vuelca su razonamiento en
`content`, así que el JSON no parsea. Es un fallo de formato disfrazado de día tranquilo.

**Lo que hicimos.** El extractor marca el *vacío sospechoso*: si el texto tiene señales de
interrupción —una alcaldía nombrada, la palabra suspensión— y el modelo no devuelve nada, eso es
un error explícito. `/refresh` lo reporta en su respuesta, y la interfaz lo muestra.

**Resuelto.** El fallo silencioso ya no es silencioso.

### 3.2 `temperature=0` no garantiza determinismo

**El problema.** Con temperatura 0.1 el extractor devolvía cero eventos para el mismo aviso
entre el 25% y el 83% de las veces. Bajarla a 0 lo redujo, **no lo eliminó**:
...
Si la medición se hubiera hecho una sola vez, la conclusión habría sido exactamente la
contraria.

**Lo que hicimos.** El vacío sospechoso se reintenta **escalando al nivel de razonamiento**, no
...
**El problema.** El 24 de septiembre se afirmó —en este repositorio y en varios mensajes de
commit— que las consultas con acento hacían que Tavily devolviera resultados sin contenido. **Era
falso.** Se concluyó de un A/B de cuatro muestras que capturó una racha.

Al volver a medir: la misma consulta con acento devolvió contenido completo en **45 de 45**
resultados.

**La causa real** era degradación intermitente de Tavily, que responde `200 OK` con
`raw_content` en `null`, y un collector que aceptaba el recorte como si fuera el aviso.

**Lo que hicimos.** Se corrigió por escrito, y la lección subió a las reglas del proyecto: medir
una tasa sobre muchas corridas en lugar de leer causalidad en cuatro muestras. El mismo error se
había cometido antes con la temperatura.

**Resuelto**, y es el hallazgo del que más se aprendió.

### 3.4 Mi propio contrato dejó pasar el fallo que existía para atrapar
...
**Lo que hicimos.** Regla dura: **probar la superficie observable, no el ayudante interno.** Lo
que llega a la API, lo que devuelve `extract()`, lo que responde `/refresh`, el código de salida
de un script.

**Resuelto**, y es la razón de que hoy haya 2,285 líneas de prueba contra 912 de aplicación.

### 3.5 La métrica de precisión iba a mentir en el pitch

**El problema.** El reporte crudo dice `start ≈ 0.10` y `end ≈ 0.30`. Citarlo sería un error en
las dos direcciones:

- **`start` no mide precisión, mide abstención.** Las 10 etiquetas tienen `start` nulo, porque la
  convención prohíbe inferir un inicio que el aviso no declara. Lo que el número revela es que el
  modelo **inventa una fecha de inicio en 21 de 30 casos**.
- **`end` se hunde por una discrepancia de convención.** A nivel de día el acierto es **15 de
  15**; con la tolerancia de ±60 minutos baja a 9 de 15. Toda la diferencia es que el modelo
  devuelve `00:00:00` donde la etiqueta usa `23:59:59`.

Antes de eso, una comparación literal de nombres puso la precisión de alcaldía en **0.00** porque
el aviso decía `Benito Juarez` y la etiqueta `Benito Juárez`.

**Lo que hicimos.** Normalizar nombres contra la lista oficial de las 16 alcaldías, y documentar
la métrica **con sus distorsiones** en `docs/EVALUATION.md` en lugar de publicar el número
redondo.

**Resuelto.** Ver §5 para lo que el número dice de verdad.
...
**Lo que hicimos.** La pertenencia la decide el código, no el buscador: un resultado que no
nombre la ciudad, su organismo de agua o alguna de las 16 alcaldías se descarta y se cuenta.
Vuelto a medir: **2 resultados fuera → 0**, en ambas corridas.
...
- **El límite de una corrida por sesión no limitaba nada.** El botón decía "Corrida ya
  ejecutada" y seguía funcionando: una línea posterior lo rehabilitaba. Se detectó **probando en
  producción con el navegador**, no revisando el diff.
...
- **Cada corrida descarta 6 avisos reales.** Los 6 resultados degradados de cada búsqueda son
  publicaciones de Facebook e Instagram, varias de ellas avisos legítimos de la ciudad. Tavily
  los devuelve sin contenido y el filtro los rechaza, con razón: un titular no es un aviso.
...
**Del producto**, sobre 6 avisos reales etiquetados a mano y 3 corridas:

| | |
|---|---|
| Alcaldía | 30/30 · 1.00 |
| Colonias | 188/189 · 0.99 |
| Día de restablecimiento | 15/15 · 1.00 |
| Filas descartadas por malformación | 0 |
| Debilidad real | inventa fecha de inicio en 21 de 30 |

**Del proceso:**

| | |
|---|---|
| Commits | 61 en el repositorio de trabajo, 10 en el público |
| Líneas de aplicación | 912 |
| Líneas de prueba | 2,285 |
| Pruebas automáticas | 136, todas en verde |
| Tareas despachadas a un agente constructor | 7, con 5 modelos distintos |
| Tokens de entrada de los despachos documentados | ~8.2 millones, con 94–96% servido desde caché |
| Duración de un despacho | de 86 s a 25 min según el modelo |

## 6. Lo que nos llevamos del método
...
3. **Una sola medición no dice nada.** Nos costó dos veces: con la temperatura y con los acentos.
